...
# Metrics function (classification only)
def compute_metrics(eval_pred):
    logits1 = eval_pred.predictions
    labels1 = eval_pred.label_ids

    preds1 = np.argmax(logits1, axis=-1)

    return {
        "accuracy": accuracy_score(labels1, preds1),
        "precision": precision_score(labels1, preds1, average="macro", zero_division=0),
        "recall": recall_score(labels1, preds1, average="macro", zero_division=0),
        "f1": f1_score(labels1, preds1, average="macro", zero_division=0),
    }
